...
import edu.cs3500.spreadsheets.provider.view.WorksheetEditorVisualView;
...
public class BeyondGood {
  ...
  public static void main(String[] args) {
    ...
    if (args.length == 1 && args[0].equals("-gui")) {
      ...
      } else if (args.length == 4 && args[3].equals("-provider") && args[2].equals("-edit")) {
        // allows user to select and edit cells
        WorksheetAdapter wsAdapter = new WorksheetAdapter(model);
        edu.cs3500.spreadsheets.provider.view.WorksheetView theirView =
                new WorksheetEditorVisualView(wsAdapter, "BeyondGood");
        theirView.renderView();
        ControllerAdapter theirController = new ControllerAdapter(model,theirView);
        theirController.goController();
