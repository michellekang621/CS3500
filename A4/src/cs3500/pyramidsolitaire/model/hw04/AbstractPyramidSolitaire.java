...
/**
 * Represents an abstract class to implement PyramidSolitaireModel.
 */
public abstract class AbstractPyramidSolitaire implements PyramidSolitaireModel<Card> {
  ...
  /**
   * Default constructor for AbstractPyramidSolitaire.
   */
  public AbstractPyramidSolitaire(GameState gameState, List<List<Card>> pyramid,
                                  LinkedList<Card> stock, int maxRows, int maxDraw, Card[] draw) {
   ...
   */
  public static final class PSBuilder {
    ...
    PSBuilder gameState(GameState gameState) {
      this.gameState = gameState;
      return this;
    }

    PSBuilder pyramid(List<List<Card>> pyramid) {
      this.pyramid = pyramid;
      return this;
    }

    PSBuilder stock(LinkedList<Card> stock) {
      this.stock = stock;
      return this;
    }

    PSBuilder maxRows(int maxRows) {
      this.maxRows = maxRows;
      return this;
    }

    PSBuilder maxDraw(int maxDraw) {
      this.maxDraw = maxDraw;
      return this;
    }

    PSBuilder draw(Card[] draw) {
      this.draw = draw;
      return this;
    }

    // todo
    AbstractPyramidSolitaire build() {
      return new AbstractPyramidSolitaire(this) {};
    }
  }
   ...
   */
  private AbstractPyramidSolitaire(PSBuilder builder) {
  ...
  public void remove(int row1, int card1, int row2, int card2) throws IllegalStateException {
    ...
    if (!isGameOver()) {
      ...
      if (invalidCards(row1, card1, row2, card2)) {
        throw new IllegalArgumentException("You can't remove these cards.");
      } else {
  ...
  public void remove(int row, int card) throws IllegalStateException {
    ...
    if (!isGameOver()) {
      ...
      if (invalidCard(row, card) || invalidCombo(this.getCardAt(row, card), null)) {
        throw new IllegalArgumentException("You can't remove this card.");
      } else {
  ...
  public void discardDraw(int drawIndex) throws IllegalStateException {
    ...
    if (!isGameOver()) {
      ...
      if (invalidDrawIdx(drawIndex)) {
        throw new IllegalArgumentException("You can't discard this card.");
      } else {
        this.updateDrawPile(drawIndex);
      }
  ...
  public int getScore() throws IllegalStateException {
    ...
    for (int r = 0; r < this.maxRows; r++) {
      for (int i = 0; i < this.pyramid.get(r).size(); i++) {
  ...
  public Card getCardAt(int row, int card) throws IllegalStateException {
    ...
    if (this.invalidCoordinates(row, card)) {
      throw new IllegalArgumentException("This is an invalid card coordinate.");
    }
