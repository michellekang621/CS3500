...
public class BasicPyramidSolitaireTest {
  ...
  private List<Card> fullDeckCopy;
  ...
  private void reset() {
    ...
    this.fullDeckCopy = new ArrayList<>();
    List<List<Card>> unshuffledPyramid = new ArrayList<>();
    ...
    this.h5 = new Card(Symbol.Heart, Value.Five);
    ...
    this.fullDeckCopy.addAll(this.fullDeck);

    this.drawCards = new ArrayList<>();
    this.drawCards.add(this.h3);
    this.drawCards.add(this.h4);
    this.drawCards.add(h5);
    ...
    for (int i = 0; i < 7; i++) {
      ...
      for (int j = 0; j <= i; j++) {
        row.add(fullDeck.get(counter));
        counter++;
      }
      unshuffledPyramid.add(row);
    }
  }
  ...
  public void startGameFive() {
    this.reset();
    this.bps.startGame(null, true, 10, 3);
  }
  ...
  public void startGameSix() {
    this.reset();
    this.bps.startGame(null, true, 6, -1);
  }

  // everything is valid & shuffled
  @Test
  public void startGameSeven() {
    this.reset();
    this.bps.startGame(this.fullDeck, true, 7, 3);
    assertNotEquals(this.drawCards, this.bps.getDrawCards());
    assertEquals(7, this.bps.getNumRows());
    assertEquals(3, this.bps.getNumDraw());
  ...
  public void startGameEight() {
    ...
    this.deck.add(this.h4);
    this.deck.add(this.h5);
